          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="spuSaleAttr in spuSaleAttrList" :key="spuSaleAttr.id">
                ...
                <dd
                  changepirce="0"
                  :class="{ active: skuColor.isChecked == 1 }"
                  v-for="(skuColor, index) in spuSaleAttr.spuSaleAttrValueList"
                  :key="index"
                  @click="changeActive(skuColor,spuSaleAttr.spuSaleAttrValueList)"
                >
                  {{ skuColor.saleAttrValueName }}
                </dd>
              </dl>
            </div>
            <div class="cartWrap">
...
export default {
  ...
  computed: {
    ...
    //给子组件的数据
    skuImageList() {
      //如果服务器没有数据回来，skuInfo对象是空对象
      return this.skuInfo.skuImageList || [];
    },
  },
  methods:{
    //产品的售卖属性切换高亮
    changeActive(skuColor,attr){
      //遍历全部售卖属性值无高亮了
        attr.forEach(item => {
          item.isChecked = '0'
        });
        skuColor.isChecked = 1
    },
    //修改产品个数
    changeSkuNum(e){
